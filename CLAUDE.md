...
- **Drop System** — `Balance.DROPS` + `ADAPTIVE_DROPS` + `DROP_SCALING` + `ADAPTIVE_DROP_BALANCER`. Tier rates **STRONG/MEDIUM/WEAK** 3%/2.5%/1% (by currency symbol). Pity cycle-scaled: `PITY_BASE:30 - (cycle-1)×2`, floor 15 (C1=30/C2=28/C3=26), adjusted ±10/+5 da APC. 3 categorie: SPECIAL/UTILITY/PERK (UPGRADE **non** droppa da nemici dal v5.11 — solo boss Evolution Core). Adaptive Drop Balancer (struggle 40s/55s force, ceiling 0.40; domination 1.5 kills/s, floor 0.60, chance×0.25, pity×2; post-death grace 60s; Arcade ×0.85). Guaranteed SPECIAL pre-boss (`GUARANTEED_SPECIAL_WAVE:4`). Anti-cluster 6s hardcoded. GODCHAIN/HYPER drop suppression. Full spec: [design/gdd/drop-system-apc.md](design/gdd/drop-system-apc.md)
...
- **Adaptive Power Calibration** — `Balance.ADAPTIVE_POWER`. At cycle transitions (C2+, mai su C1), snapshots power score `ps = 0.50×weapon + 0.30×perks + 0.20×special`. HP mult `0.85 + ps×0.50` (range 0.85–1.35×). Pity adjust: -10 se `ps<0.30`, +5 se `ps>0.60`. ⚠️ **Tre formule power-score coesistenti**: APC 0.50/0.30/0.20, ADAPTIVE_DROPS 0.65/0.35 (no perks), Balancer 0.50/0.25/0.25 — consolidare in futuro. Debug: sezione APC in `dbg.report()`. Full spec: [design/gdd/drop-system-apc.md](design/gdd/drop-system-apc.md)
