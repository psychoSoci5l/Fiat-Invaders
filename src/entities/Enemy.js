...
class Enemy extends window.Game.Entity {
    constructor(x, y, typeConf) {
        ...
        // v7.14: Fire Burn DoT — damage over time applied by fire elemental perk
        this._burnDotActive = false;
        this._burnDotDuration = 0;
        this._burnDotTickTimer = 0;
        this._burnDotTickDamage = 0;
        this._burnKilled = false;

        // v7.14: Electric Stun — freeze enemy movement and firing
        this._stunTimer = 0;
    ...
    update(dt, globalTime, wavePattern, gridSpeed, gridDir, playerX, playerY) {
        ...
        // v7.14: Burn DoT tick processing (fire elemental)
        if (this._burnDotActive && this._burnDotDuration > 0) {
            this._burnDotDuration -= dt;
            this._burnDotTickTimer -= dt;
            if (this._burnDotTickTimer <= 0) {
                const dotCfg = window.Game.Balance?.ELEMENTAL?.FIRE?.BURN_DOT;
                this._burnDotTickTimer = dotCfg?.TICK_INTERVAL || 0.45;
                this.hp -= this._burnDotTickDamage;
                this.hitFlash = 1;
                if (this.hp <= 0) {
                    this._burnKilled = true;
                    this._burnDotActive = false;
                }
            }
            if (this._burnDotActive) {
                this._elemTint = window.Game.Balance?.ELEMENTAL?.ENEMY_TINT?.FIRE || '#ff4400';
                this._elemTintTimer = 1.0;
                this._elemPersistent = true;
            }
        }

        // v7.14: Electric stun — skip movement and firing when stunned
        if (this._stunTimer > 0) {
            this._stunTimer -= dt;
            this._elemTint = window.Game.Balance?.ELEMENTAL?.ENEMY_TINT?.ELECTRIC || '#8844ff';
            this._elemTintTimer = this._stunTimer;
            this._elemPersistent = false;
            if (this.hitFlash > 0) this.hitFlash -= dt * 3;
            if (this._hitShakeTimer > 0) {
                this._hitShakeTimer -= dt;
                if (this._hitShakeTimer > 0) {
                    const si = (window.Game.Balance?.VFX?.HIT_SHAKE_INTENSITY || 2) * (this._hitShakeTimer / 0.06);
                    this._hitShakeX = (Math.random() - 0.5) * 2 * si;
                    this._hitShakeY = (Math.random() - 0.5) * 2 * si;
                } else {
                    this._hitShakeX = 0;
                    this._hitShakeY = 0;
                }
            }
            return; // Skip movement, pattern, fire processing while stunned
        }
        ...
        if (this._v8Fall) {
            ...
            // Off-screen cull: remove enemies that fly far above the screen
        ...
        if (this.behavior && behCfg?.ENABLED) {
                ...
                if (this._behaviorPhase === 'RUN') {
                    ...
                    if (this._flankerFireTimer <= 0 && playerY !== undefined) {
                        ...
                        if (window.Game.Events) {
                            ...
                            if (bd) window.Game.Events.emit('system:harmonic-bullets', { bullets: [bd] });
                        }
                    }
            ...
            if (this.behavior === 'BOMBER' && behCfg.BOMBER?.ENABLED) {
                ...
                if (this._behaviorTimer <= 0) {
                    ...
                    if (window.Game.Events) {
                        window.Game.Events.emit('enemy:bomber-drop', {
    ...
    draw(ctx) {
        ...
        if (this.isMinion) {
            ...
            var _rendered = G.EnemyAgentRenderer.drawAgent(ctx, this, x, y);
            if (!_rendered) {
                this.drawMinion(ctx, x, y);
            }
        }
    ...
    // v7.13.1: delegated to EnemyAgentRenderer
    drawGlow(ctx) {
        G.EnemyAgentRenderer.drawGlow(ctx, this);
    }
