...
function checkFile(filePath) {
    ...
    // Does it use G as an object reference?
    ...
    const declaresG = /(?:^|;|\s)(?:const|let|var)\s+G\b/.test(code) ||
                      ...
                      /(?:const|let|var)\s+G\s*=\s*window\.Game/.test(code);
    ...
    if (!declaresG) {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (/\bG\b\./.test(lines[i])) {
                matches.push(i + 1);
            }
        }
        issues.push({ file: path.relative('/home/psychosocial/Documenti/Claude Studios/fiatvscrypto', filePath), lines: matches });
    }
}
...
if (issues.length === 0) {
    ...
    for (const i of issues) {
        console.log(`  ${i.file} (lines: ${i.lines.join(', ')})`);
    }
}
